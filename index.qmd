...
df <- mutate(df, penguin=str_squish(df$penguin))
...
-   The [Long Term Ecological Research (LTER)]{style="color:#edea2b"} program was started by the National Science Foundation in the late 70's. Data from LTER are publicly available.

-   One study recorded the size of fiddler crabs in 2016 in salt marshes from Florida to Massachusetts.

-   First, load [crabs.csv]{style="color:#edea2b"} in the data folder and try the following:

    1. Using piping methods, `tidy` the data.
    2. Summarize crab size by site.

::: fragment
-   Next, determine if the data supports ["Bergmann’s rule"]{style="color:#edea2b"}, which predicts that organisms at higher latitudes are larger than ones at lower latitudes, by finishing the following script:
...
## Exercise 2 - Data wrangling: {data-background="#ffffff" data-menu-title="Exercise1"}
-   How did you do?
